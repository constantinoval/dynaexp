...
from libs.datastorage.tables import ElasticProperties, Jacket
from libs.common_tools import to_float


class Add_Jacket_Dlg(Ui_Dialog_Add_Striker, pqw.QDialog):
    def __init__(self, session, instance=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.setupUi(self)
        self.setModal(True)
        self.setWindowTitle("Добавление обоймы")
    ...
    def accept(self):
        ...
        if self.instance:
            ...
            s = Jacket(D=D, D0=D0, L=L, material=m)
            s.set_creation_time()
            if comment := str(self.comment_text.toPlainText()):
                s.notes = comment
            self.session.add(s)
